...
public class HuffmanCoding {
    ...
    public static ArrayList<CharFreq> makeSortedList(String filename) {
        ...
        for (int i = 0; i < frequencyarr.length; i++) {
            if (frequencyarr[i] > 0) {
                ...
                x.setCharacter((char) i);
                x.setProbOccurrence((double) (frequencyarr[i])/totalfreq);
                list.add(x);
            }
        }
        for (int i = 0; i < frequencyarr.length - 1; i++) {
            if (list.size() == 1 && frequencyarr[i] > 0) {
                CharFreq x = new CharFreq();
                x.setCharacter((char) (i+1));
                x.setProbOccurrence((double) (frequencyarr[i + 1])/totalfreq);
                list.add(x);
            }
        }
    ...
    public static TreeNode makeTree(ArrayList<CharFreq> sortedList) {
        ...
        while (sortedList.size() != 0) {
            TreeNode x = new TreeNode(sortedList.remove(0), null, null);
            source.enqueue(x);
        }
        ...
        while (!source.isEmpty() || target.size() > 1) {
            ...
            CharFreq c = new CharFreq(null, left.getData().getProbOccurrence() + right.getData().getProbOccurrence());
            root = new TreeNode(c, left, right);
            target.enqueue(root);
        }
    ...
    public static String[] makeEncodings(TreeNode root) {
        ...
        String temp = "";
        encode(root,temp,output);

        return output; // Delete this line
    }
    private static void encode(TreeNode root, String tempString, String[] arr) {
        
        if(root.getData().getCharacter() != null) {
            arr[(int) root.getData().getCharacter()] = tempString;
            
        }
        else {
            encode(root.getLeft(),tempString + "0", arr);
            encode(root.getRight(),tempString +"1", arr);
        }
    }
    /**
