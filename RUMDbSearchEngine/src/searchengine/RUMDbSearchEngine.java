...
public class RUMDbSearchEngine {
	...
	public void insertMoviesIntoHashTable ( String inputFile ) {
		...
		for (ArrayList<String> individualmovie: movies) {
			String title = individualmovie.get(0);
			for (int j = 1; j < individualmovie.size(); j++) {
				String descriptionword = isWord(individualmovie.get(j));
				if (descriptionword != null) {
					Location loc = new Location(title, j);
					insertWordLocation(descriptionword, loc);
				}
			}
		}
	}
	...
	public void insertWordLocation (String word, Location loc) {
        ...
        // COMPLETE THIS METHOD
		if (getWordOccurrence(isWord(word)) != null) {
			getWordOccurrence(isWord(word)).addOccurrence(loc);
	...
	public WordOccurrence getWordOccurrence (String word) {
        ...
        // COMPLETE THIS METHOD
	   WordOccurrence ptr = hashTable[hashFunction(word)];
	   while (ptr != null) {
	...
	public ArrayList<MovieSearchResult> createMovieSearchResult (String wordA, String wordB) {
        ...
        // COMPLETE THIS METHOD
		ArrayList<MovieSearchResult> out = new ArrayList<MovieSearchResult>();
		
		WordOccurrence WordAs = getWordOccurrence(wordA);
		WordOccurrence WordBs = getWordOccurrence(wordB);
		if (WordAs == null || WordBs == null) {
			return null;
		}
		ArrayList <Location> first = WordAs.getLocations();
		ArrayList <Location> second = WordBs.getLocations();
		for(int i = 0; i < first.size(); i++) {
			Location x = first.get(i);
			MovieSearchResult temp = new MovieSearchResult(x.getTitle());
			temp.addOccurrenceA(x.getPosition());

			while(i+1 < first.size() && first.get(i+1).getTitle().equals(x.getTitle())) {
				temp.addOccurrenceA(first.get(i++).getPosition());
			}

			for(Location var: second) {
				if (var.getTitle().equals(x.getTitle())) {
					temp.addOccurrenceB(var.getPosition());
				}
			}
			out.add(temp);
		}
		...
		for(Location var: second) {
			int z = 0;
			for (MovieSearchResult res: out) {
				if (var.getTitle().equals(res.getTitle())) {
					z++;
				}
			}
			if (z ==0) {
				MovieSearchResult temp = new MovieSearchResult(var.getTitle());
				temp.addOccurrenceB(var.getPosition());
				out.add(temp);
			}
	...
	public void calculateMinDistance(MovieSearchResult msr){
		...
		if (msr.getArrayListA() == null || msr.getArrayListB() == null || msr.getArrayListA().size() == 0 || msr.getArrayListB().size() == 0) {
		...
		int previousA = loca.get(counta);
		int previousB = locb.get(countb);
		int minDistance = Integer.MAX_VALUE;
		while (counta < loca.size() && countb < locb.size()) {
			int dis = Math.abs(previousA - previousB);
			if (minDistance > dis) {
				minDistance = dis;
			}
			if (counta < loca.size() - 1 && countb < locb.size() - 1) {
				if (previousA < previousB) {
					previousA = loca.get(++counta);
				}
				else {
					previousB = locb.get(++countb);
				}
			}
			else {
				counta++;
	...
	public ArrayList<MovieSearchResult> topTenSearch(String wordA, String wordB){
        ...
        // COMPLETE THIS METHOD
		ArrayList<MovieSearchResult> input = createMovieSearchResult(wordA, wordB);
		if (input == null) {
			return null;
		}
		for (MovieSearchResult a: input) {
			calculateMinDistance(a);
		}
		Collections.sort(input);
		
		ArrayList<MovieSearchResult> x = new ArrayList<MovieSearchResult>();
		int i = 0;
			while (i < 10 && i != input.size()) {
				if (input.get(i).getMinDistance() != -1) {
					x.add(input.get(i));
				}
				i++;
		}
